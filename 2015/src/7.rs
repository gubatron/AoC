...
fn part1() -> i32 {
    ...
    for i in instructions {
        ...
        if let Ok(value) = instruction_string.parse::<u16>() {
            ...
            else if gate_has_signal(&input1.to_string(), &circuit_map)
                && gate_has_signal(&input2.to_string(), &circuit_map)
            {
                let gate1 = circuit_map.get(&input1.to_string()).unwrap();
                let gate2 = circuit_map.get(&input2.to_string()).unwrap();
                let val1 = gate1.signal.unwrap();
    ...
    for id in ids_to_update {
        ...
        if gate_has_signal_on_both_inputs(&id, &circuit_map) {
            ...
            let g2 = circuit_map
                .get(&gate.input2.as_ref().unwrap().to_string())
                .unwrap();
            signal = evaluate_gate(
                g1.signal.unwrap(),
                g2.signal.unwrap(),
                gate.instruction.unwrap(),
            );
        }
